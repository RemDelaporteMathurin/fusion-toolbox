...
def plot_shot(shot_number):
    ...
    axs[0].plot(time_points, plasma_current_points, label='Plasma Current (A)')
    axs[0].set_ylabel('Plasma Current (A)')
    axs[0].legend(loc='upper left')

    # Subplot 2: Plasma Density
    axs[1].plot(time_points, plasma_density_points, label='Plasma Density (particles/m^3)')
    axs[1].set_ylabel('Plasma Density (particles/m^3)')
    axs[1].legend(loc='upper left')

    # Subplot 3: Plasma Temperature
    axs[2].plot(time_points, plasma_temperature_points, label='Plasma Temperature (K)')
    axs[2].set_ylabel('Plasma Temperature (K)')
    axs[2].legend(loc='upper left')

    # Subplot 4: Fusion Power and Radiation Power
    axs[3].plot(time_points, fusion_power_points, label='Fusion Power (W)')
    axs[3].plot(time_points, radiation_power_points, label='Radiation Power (W)')
